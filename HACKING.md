Hacking on Atomic Enterprise
============================

## Building a Release

[//]: # (TODO: Update instructions for building releases once we solidfy the process)

[//]: # (TODO: Update the base image name when ready)

To build an Atomic Enterprise release you run the `hack/build-release.sh` script on a system with Docker, which
...
the `openshift/origin-base` image which is the common parent image for all Atomic Enterprise Docker images.
...
2. Push the tag to GitHub `git push origin --tags` where `origin` is `github.com/projectatomic/atomic-enterprise.git`


## Test Suites

Atomic Enterprise uses three levels of testing - unit tests, integration test, and end-to-end tests (much
like Kubernetes).
...
Atomic Enterprise and Kubernetes use [Godep](https://github.com/tools/godep) for dependency management.
...
Atomic Enterprise is checked into this repository.  To install `godep` locally run:
...
There are a few steps involved in rebasing Atomic Enterprise to a new version of Kubernetes. We need to make sure
...
were properly reflected* in the Atomic Enterprise codebase.

### 1. Preparation

Before you begin, make sure you have both [projectatomic/atomic-enterprise](https://github.com/projectatomic/atomic-enterprise) and [GoogleCloudPlatform/kubernetes](https://github.com/GoogleCloudPlatform/kubernetes) in your $GOPATH:

```
$ go get github.com/projectatomic/atomic-enterprise
...
### 2. Rebase Atomic Enterprise to the new Kubernetes version
...
`godep restore` from both the Atomic Enterprise and the Kubernetes directories and then `godep save ./...`
from the Atomic Enterprise directory. Follow these steps:

1. `$ cd $GOPATH/src/github.com/projectatomic/atomic-enterprise`
2. `make clean ; godep restore` will restore the package versions specified in the `Godeps/Godeps.json`
of Atomic Enterprise to your GOPATH.
...
5. `$ cd $GOPATH/src/github.com/projectatomic/atomic-enterprise`.
...
### 3. cherry-pick upstream changes pushed to the Atomic Enterprise repo

Eventually during the development cycle we introduce changes to dependencies right in the Atomic Enterprise
...
1. Check the `Godeps` directory [commits history](https://github.com/projectatomic/atomic-enterprise/commits/master/Godeps)
...
### 4. Refactor Atomic Enterprise to be compliant with upstream changes

After making sure we have all the dependencies in place and up-to-date, we need to work in the Atomic Enterprise
...
can potentially break or change any part of Atomic Enterprise, the most affected parts are usually:

1. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/cmd/server/start.go
2. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/cmd/server/kubernetes/master.go
3. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/cmd/server/origin/master.go
4. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/cmd/cli/cmd/factory.go
5. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/cmd/cli/cli.go
6. https://github.com/projectatomic/atomic-enterprise/blob/master/pkg/api/meta/multimapper.go
...
If you run into difficulties running Atomic Enterprise, start by reading through the
[troubleshooting guide](https://github.com/projectatomic/atomic-enterprise/blob/master/docs/debugging-atomic-enterprise.md).

## RPM Packaging

A specfile is included in this repo which can be used to produce RPMs including
the atomic-enterprise binary. While the specfile will be kept up to date with build
...
Atomic Enterprise and Kubernetes integrate with the [Swagger 2.0 API framework](http://swagger.io) which aims to make it easier to document and write clients for RESTful APIs.  When you start Atomic Enterprise, the Swagger API endpoint is exposed at `https://localhost:8443/swaggerapi`. The Swagger UI makes it easy to view your documentation - to view the docs for your local version of Atomic Enterprise start the server with CORS enabled:

    $ atomic-enterprise start --cors-allowed-origins=.*
...
Note: Hosted API documentation will be available in the future.


## Performance debugging

Atomic Enterprise integrates the go `pprof` tooling to make it easy to capture CPU and heap dumps for running systems.  The following modes are available for the `atomic-enterprise` binary (including all the CLI variants):
...
* `OPENSHIFT_PROFILE` environment variable:
  ...
  * `web` - start the pprof webserver in process at http://127.0.0.1:6060/debug/pprof (you can open this in a browser)
    ...
    $ OPENSHIFT_PROFILE=cpu sudo ./_output/local/go/bin/atomic-enterprise start

To view profiles, you use [pprof] which is part of `go tool`.  You must pass the binary you are debugging (for symbols) and a captured pprof.  For instance, to view a `cpu` profile from above, you would run Atomic Enterprise to completion, and then run:

    $ go tool pprof ./_output/local/go/bin/atomic-enterprise cpu.pprof
...
`pprof` supports CLI arguments for looking at profiles in different ways - memory profiles by default show allocated space:
    ...
    $ go tool pprof ./_output/local/go/bin/atomic-enterprise mem.pprof

but you can also see the allocated object counts:

    $ go tool pprof --alloc_objects ./_output/local/go/bin/atomic-enterprise mem.pprof

Finally, when using the `web` profile mode, you can have the go tool directly fetch your profiles via HTTP:

    # for a 30s CPU trace
    $ go tool pprof ./_output/local/go/bin/atomic-enterprise http://127.0.0.1:6060/debug/pprof/profile

    # for a snapshot heap dump at the current time, showing total allocations
    $ go tool pprof --alloc_space ./_output/local/go/bin/atomic-enterprise http://127.0.0.1:6060/debug/pprof/heap
